/**
 * Food & cooking SVG icon library.
 * Each icon has small (100x100) and large (300x180) variants.
 * Uses currentColor for strokes so theme CSS controls color.
 */
const ICONS = {
    // --- Proteins ---
    beef: {
...
const _SVG_ALLOWED_ELEMENTS = new Set(['svg', 'g', 'path', 'circle', 'ellipse', 'rect',
    'line', 'polyline', 'polygon', 'text', 'tspan', 'defs', 'use',
...
const _SVG_ALLOWED_ATTRS = new Set([
    ...
    'xlink:href', 'href', 'xmlns', 'xmlns:xlink', 'version',
    'marker-start', 'marker-mid', 'marker-end',
    'visibility', 'display', 'overflow', 'clip-path', 'mask',
    'filter', 'color',
]);
...
function sanitizeSVG(svgString) {
    ...
    for (const el of [...doc.querySelectorAll('*')]) {
        if (!_SVG_ALLOWED_ELEMENTS.has(el.localName)) { el.remove(); continue; }
...
function registerCustomIcon(key, svg) {
    _customIconCache[key] = sanitizeSVG(svg);
}

function removeCustomIcon(key) {
    delete _customIconCache[key];
}

function getCustomIconKeys() {
...
/** Returns the small (100×100) SVG string for the given key, or the chef-hat fallback */
...
/** Returns the large (300×180) SVG string for the given key, or the chef-hat fallback */
function getLargeIcon(key) {
    const entry = ICONS[key];
    if (entry) return entry.large;
    return ICONS['chef-hat'] ? ICONS['chef-hat'].large : '';
}

/** Returns the ICON_CATEGORIES object for grouped picker UI */
function getIconCategories() {
    return ICON_CATEGORIES;
}

/** Returns the large SVG for a key (built-in or custom), or null if not found */
function resolveIconLarge(key) {
    if (key && key.startsWith('custom:') && _customIconCache[key]) {
        return _customIconCache[key];
    }
    const entry = ICONS[key];
...
// ─── Constraint-type icons (admin profile builder) ─────────────
const CONSTRAINT_ICONS = {
    keyword: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20.59 13.41l-7.17 7.17a2 2 0 01-2.83 0L2 12V2h10l8.59 8.59a2 2 0 010 2.82z"/><circle cx="7" cy="7" r="1" fill="currentColor"/></svg>',
    food: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 2v4M16 2v4M12 2v4M3 10h18M5 10v10a2 2 0 002 2h10a2 2 0 002-2V10"/><path d="M12 14v4"/></svg>',
...
// ─── Eager custom-icon loading ────────────────────────────────
// Fetches custom icons on script load so they're available before
// any component (app.js, admin.js) calls getIconByKey('custom:…').
(function _loadCustomIconsEagerly() {
    fetch('/api/custom-icons/all')
        .then(r => r.ok ? r.json() : {})
        .then(svgMap => {
            for (const [key, svg] of Object.entries(svgMap)) {
                registerCustomIcon(key, svg);
            }
            // Notify Alpine components that icons are ready
            window.dispatchEvent(new CustomEvent('custom-icons-loaded'));
        })
        .catch(e => console.warn('Custom icon preload failed:', e));
})();
